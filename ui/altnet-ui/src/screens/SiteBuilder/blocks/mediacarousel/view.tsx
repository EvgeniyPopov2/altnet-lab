...
export default function MediaCarouselView({ block }: { block: any }) {
  const bx = (block as Partial<MediaCarouselLike>) || {};

  const items: MediaItem[] =
    (Array.isArray(bx.items) && (bx.items as MediaItem[])) || [
    ...
    <div className="relative">
      ...
      >
        {items.map((it: MediaItem) => (
          <div
            key={it.id}
            className="min-w-[320px] snap-center shrink-0 rounded-2xl border border-[#1f2751] bg-[#0b1022] overflow-hidden"
          >
            {it.type === "video" ? (
      ...
      <div className="hidden md:flex gap-2 absolute -top-4 right-0">
        <button
          ...
          className="px-2 py-1 rounded border border-[#1f2751] bg-[#101735] text-[#e6e9f4]"
        >
          ‹
        </button>
        <button
