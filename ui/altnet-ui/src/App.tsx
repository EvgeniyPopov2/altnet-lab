...
export default function App() {
  ...
  // Security Coach: короткая подсказка при смене профиля
  useEffect(() => {
  ...
  const openCall = (next: CallWindowModel) => {
    ...
    if (call) {
      SecurityCoach.deniedByPolicy({
        ...
        message:
          "Сейчас поддерживаем один звонок за раз. Завершите текущий звонок, чтобы начать новый.",
    ...
    () => [
      {
        id: "henk",
        title: "Henk",
        subtitle: "в сети • 5 мин назад",
        caps: { contactId: "henk", supportsAnon: true, supportsFast: true },
      },
      {
        id: "valerych",
        title: "Валерыч",
        subtitle: "anon-only • не в сети",
        caps: { contactId: "valerych", supportsAnon: true, supportsFast: false },
      },
      {
        id: "crystal",
        title: "Crystallick",
        subtitle: "fast-only • новое: 3",
        caps: { contactId: "crystal", supportsAnon: false, supportsFast: true },
      },
      {
        id: "dencoldgrey",
        title: "dencoldgrey",
        subtitle: "Пишет…",
        caps: { contactId: "dencoldgrey", supportsAnon: true, supportsFast: true },
      },
      {
        id: "curdone",
        title: "Curd_one",
        subtitle: "AFK",
        caps: { contactId: "curdone", supportsAnon: true, supportsFast: true },
      },
    ],
    []
  );
  ...
  const selectedDm = useMemo(
    () => dmList.find((d) => d.id === selectedDmId) ?? dmList[0],
    [dmList, selectedDmId]
  );
    ...
    <div className="hidden md:flex items-center gap-1 bg-white/10 rounded-xl p-1">
      <button
        ...
        className={`px-2.5 py-1 rounded-lg text-sm ${
          profile === "anon" ? "bg-white/20 text-white" : "text-white/70 hover:bg-white/15"
        }`}
      ...
      <button
        ...
        className={`px-2.5 py-1 rounded-lg text-sm ${
          profile === "fast" ? "bg-white/20 text-white" : "text-white/70 hover:bg-white/15"
        }`}
    ...
    <div className="h-screen flex overflow-hidden">
      ...
      >
        ...
        <div className="mt-auto px-4">
          <div className="w-10 h-10 rounded-full bg-gradient-to-br from-indigo-500 to-fuchsia-500 flex items-center justify-center font-bold">
            E
          </div>
          <div className="mt-2 grid grid-cols-3 gap-1">
            ...
            <button
              className="h-8 rounded-lg bg-white/10 hover:bg-white/20 text-xs"
              title="Настройки"
              onClick={openProfile}
              type="button"
            >
            ...
            <button
              className={`h-8 rounded-lg text-xs ${
                section === "profile" ? "bg-indigo-600" : "bg-white/10 hover:bg-white/20"
              }`}
              title="Профиль"
              onClick={openProfile}
              type="button"
          ...
          <div className="flex items-center gap-2">
            ...
            <button
              className={`px-3 py-1.5 rounded-full text-sm ${
                section === "mail" ? "bg-indigo-600/30 hover:bg-indigo-600/40" : "bg-white/10 hover:bg-white/20"
              }`}
              type="button"
              onClick={openMail}
              title="Открыть почту"
                    ...
                    <div className="rounded-2xl border border-white/10 bg-white/5 p-4">
                      ...
                      <div
                        className={`mt-2 text-lg font-semibold ${
                          dashboardStats.panicActive ? "text-red-300" : "text-emerald-300"
                        }`}
                      >
                ...
                <div className="rounded-2xl border border-white/10 bg-white/5 p-6 text-white/80 space-y-4">
                  <div>
                    ...
                    <div className="text-sm text-white/60 mt-1">
                      Профиль влияет на транспорт и ограничения приватности (fail-closed).
                    </div>
                  </div>
                  ...
                  <div className="flex gap-2">
                    <button
                      ...
                      className={`px-3 py-1.5 rounded-lg ${
                        profile === "anon" ? "bg-indigo-600 text-white" : "bg-white/10 hover:bg-white/20"
                      }`}
                      type="button"
                    >
                      🕶️ Анонимный
                    ...
                    <button
                      onClick={() => setProfile("fast")}
                      className={`px-3 py-1.5 rounded-lg ${
                        profile === "fast" ? "bg-indigo-600 text-white" : "bg-white/10 hover:bg-white/20"
                      }`}
                      type="button"
                    >
                      ⚡ Быстрый
                  ...
                  <div className="rounded-xl border border-white/10 bg-white/5 p-4">
                    ...
                    <div className="rounded-xl border border-white/10 bg-white/5 p-4 mt-3">
                      ...
                      <div className="mt-2 text-sm text-white/70">
                        Настройки микрофона, видео и режимов ввода (VAD/PTT). Часть опций может блокироваться policy
                        профиля.
                      </div>
                    ...
                    <div className="mt-3 text-sm text-white/70">
                      Если вы подозреваете компрометацию устройства — включите «Панику». Это заблокирует сетевые действия
                      и покажет чек‑лист.
                    </div>
      ...
      <VoiceVideoSettingsModal open={voiceVideoOpen} profile={profile} onClose={() => setVoiceVideoOpen(false)} />
    </div>
  );
}