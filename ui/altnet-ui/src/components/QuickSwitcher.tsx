  ...
  return (
    <AnimatePresence>
      {open && (
        ...
        >
          <motion.div
            className="w-[680px] rounded-2xl border border-white/10 bg-neutral-900 shadow-xl overflow-hidden"
          ...
          >
            ...
            <div className="max-h-[420px] overflow-y-auto p-2">
              {filtered.length === 0 && (
                <div className="px-3 py-6 text-sm text-white/60">Ничего не найдено. Попробуйте другой запрос.</div>
              )}
              {filtered.map((it, i) => (
                <button
                  key={it.id}
